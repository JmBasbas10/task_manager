        ...
        <div class="card border-0 shadow-sm h-100" style="border-radius: 12px;">
            <div class="card-body p-4">
                ...
                <div class="d-flex flex-wrap gap-2 mb-4">
                    ...
                    <span class="badge bg-{{ $pColor }} fs-6 px-3 py-2">
                ...
                @if($task->description)
                    <div class="mb-0">
                        ...
                        <p class="lh-lg mb-0">{{ $task->description }}</p>
                    </div>
                @else
                ...
                <div class="d-flex flex-column gap-2">
                    @foreach(['todo' => ['To Do', 'secondary'], 'in_progress' => ['In Progress', 'primary'], 'done' => ['Done', 'success']] as $val => [$label, $color])
                        <form action="{{ route('tasks.status', $task) }}" method="POST">
                            ...
                            <button type="submit"
                                class="btn btn-{{ $task->status === $val ? $color : 'outline-'.$color }} btn-sm w-100 text-start {{ $task->status === $val ? 'disabled' : '' }}"
